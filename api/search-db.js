        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (searchInput && searchDropdown) {
                ...
                const loadArticles = async () => {
                    if (allArticles !== null || isLoading) return;
                    isLoading = true;
                    try {
                        const res = await fetch('/api/search-db?t=' + Date.now());
                        allArticles = await res.json();
                    } catch (e) { 
                        console.error("Ошибка загрузки базы поиска", e);
                        allArticles = [];
                    }
                    isLoading = false;
                ...
                searchInput.addEventListener('focus', loadArticles);
                ...
                searchInput.addEventListener('input', function() {
                    ...
                    if (allArticles === null) {
                        searchDropdown.innerHTML = '<div style="padding: 10px 15px; color: #64748b; font-size: 14px;">Загрузка базы...</div>';
